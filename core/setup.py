...
from eitcogs.utils import userinput
...
async def setup_dialog(eitcog, member):
    ...
    while True:
        name = await userinput(eitcog.bot, member.dm_channel, member)
        if is_valid(name):
            break
        else:
    ...
    await member.send(embed=setup_group_select(name, eitcog.semesters))

    # loop until User tiped in a valid studygroup
    flag = True
    roles_to_add = []
    while flag:
        message = await userinput(eitcog.bot, member.dm_channel, member)
        ...
        for group in eitcog.study_groups:
            ...
            await member.send(embed=setup_group_error(message))


async def remove_groups(eitcog, member):
    for role in member.roles:
        if role in [group.role for group in eitcog.study_groups]:
